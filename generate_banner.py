...
def main():
    # 1. Read the Mario character from the old banner file
    try:
        with open('old_banner_utf8.svg', 'r', encoding='utf-8') as f:
            old_svg = f.read()
    except FileNotFoundError:
        print("Error: old_banner_utf8.svg not found. Run previous extraction command.")
        return

    # Extract Mario character block
    mario_match = re.search(
        r'(<!-- ========= MARIO CHARACTER \(static position\) ========= -->.*?)(<!-- ========= TEXT OVERLAY ========= -->)', 
        old_svg, 
        re.DOTALL
    )
    if not mario_match:
        print("Error: Could not extract Mario character from old_banner_utf8.svg")
        return
    mario_char_block = mario_match.group(1).strip()
